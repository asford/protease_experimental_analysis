...
class FractionalSelectionModel(object):
    ...
    def build_model(self, population_data):
        for k, p in population_data.items():
            unused_keys = set(p.keys()).difference( {"selected", "fraction_selected", "selection_level", "parent", "min_fraction"} )
            if unused_keys:
        ...
        with self.model:
            sel_k = pymc3.Uniform("sel_k", lower=0.5, upper=5.0, shape=1, testval=2.0)#start_k)
            sel_k_transform = pymc3.distributions.transforms.Interval(0.5, 5.0).forward(sel_k)
            self.to_trans["sel_k"] = ("sel_k_interval_"), lambda v: sel_k_transform.eval({sel_k : v})

            sel_ec50 = pymc3.Uniform("sel_ec50", lower=0.001, upper=10.0, shape=self.num_members, testval=0.5)#start_k)
            sel_ec50_transform = pymc3.distributions.transforms.Interval(0.001, 10.0).forward(sel_ec50)
            ...
            for pkey in pops_by_depth:
                ...
                if pdat["selection_level"] is not None:
                    ...
                    selection_dist = start_dist * (( 1.0 + T.erf( sel_k - (sel_ec50 * ( 3.0 ** T.constant(pdat["selection_level"])  ))  ) ) / 2.0)       
